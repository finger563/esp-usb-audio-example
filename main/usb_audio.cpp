...
extern "C" bool tud_audio_set_req_ep_cb(uint8_t rhport, tusb_control_request_t const * p_request, uint8_t *pBuff)
{
  ...
  logger.debug("Set request for EP");
...
extern "C" bool tud_audio_set_req_itf_cb(uint8_t rhport, tusb_control_request_t const * p_request, uint8_t *pBuff)
{
  ...
  logger.debug("Set request for interface");
...
extern "C" bool tud_audio_set_req_entity_cb(uint8_t rhport, tusb_control_request_t const * p_request, uint8_t *pBuff)
{
  ...
  logger.debug("Set request for entity");
  ...
  // from tinyusb/examples/device/cdc_uac2/src/uac2_app.c
...
extern "C" bool tud_audio_get_req_ep_cb(uint8_t rhport, tusb_control_request_t const * p_request)
{
  ...
  logger.debug("Get request for EP");
...
extern "C" bool tud_audio_get_req_itf_cb(uint8_t rhport, tusb_control_request_t const * p_request)
{
  ...
  logger.debug("Get request for interface");
...
extern "C" bool tud_audio_get_req_entity_cb(uint8_t rhport, tusb_control_request_t const * p_request)
{
  ...
  logger.debug("Get request for entity");
...
static bool tud_audio_feature_unit_get_request(uint8_t rhport, audio_control_request_t const *request)
{
  ...
  else if (UAC2_ENTITY_SPK_FEATURE_UNIT && request->bControlSelector == AUDIO_FU_CTRL_VOLUME)
  {
    ...
    {
      audio_control_range_2_n_t(1) range_vol = {0};
      range_vol.wNumSubRanges = tu_htole16(1);
      range_vol.subrange[0] = {0};
    ...
    {
      audio_control_cur_2_t cur_vol = { .bCur = (int16_t)tu_htole16(volume[request->bChannelNumber]) };
